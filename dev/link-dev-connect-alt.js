...
/**
function compareTilesForOptions(movementOptionNorth, movementOptionEast, movementOptionSouth, movementOptionWest){
  console.log(movementOptionNorth);
  console.log(movementOptionEast);
  console.log(movementOptionSouth);
  console.log(movementOptionWest);
  tileGridOptionsID = hyruleGridTilePrepend + htmlScreenGridTileID + "Options";
  console.log(tileGridOptionsID);
  tileGridOptionsValues = hyruleGridOptionTiles[tileGridOptionsID];
  console.log(tileGridOptionsValues.door.gridPosition);
  // 
  console.log("now: " +movementOptionNorth+ " " +tileGridOptionsValues.door.gridPosition);
  if(movementOptionNorth[0] === tileGridOptionsValues.door.gridPosition[0]) {
    console.log("x val match");
    if(movementOptionNorth[1] === tileGridOptionsValues.door.gridPosition[1]) {
      console.log("y val match");
      return true;
    }
  }
  return false;
...
function getScreenGridTileOptions(arr){
  tileGridOptionsID = hyruleGridTilePrepend + htmlScreenGridTileID + "Options";
  console.log(tileGridOptionsID);
  tileGridOptionsValues = hyruleGridOptionTiles[tileGridOptionsID];
  console.log(tileGridOptionsValues.door.gridPosition);
  var optionXOpt = parseInt(arr[0]);
  console.log(optionXOpt+ ",  " +typeof optionXOpt+ ", "+tileGridOptionsValues.door.gridPosition[0]+ ", " +typeof tileGridOptionsValues.door.gridPosition[0]);
  var optionYOpt = parseInt(arr[1]);
  if(optionXOpt === tileGridOptionsValues.door.gridPosition[0]) {
    console.log("inner x");
    if(optionYOpt === tileGridOptionsValues.door.gridPosition[2]){
      console.log("inner y");
    }
  }
  
  //console.log(parseInt(arr[0]));
  //compareScreenGridTileOptions
  //if(parseInt(arr[0]) === ){
  //}
  console.log(arr[1]);
  console.log(arr[2]);
...
function getAdjacentHyruleTiles() { // get n, e, s, w hyrule tile grids adjacent to current active tile grid gameplay is on
  ...
  let fakeHyruleGridColumnWest = hyruleGridColumn;
  //  console.log("fake column west: " +fakeHyruleGridColumnWest);
  ...
  //  console.log("zero option west y deuce: " +zeroOptionWestY);
    ...
    zeroOptionWestY = zeroOptionWestY.toString();
  //  console.log("zero option west y value if greater than 9!");
  }
...
//  console.log("toe: " +tileOptionEast+ ", toe0: " +hyruleGridRow+ ", toe1: " +zeroOptionEastY);
...
//  console.log("current grid row: " +currentGridRow+ ", current grid column: " +currentGridColumn);
  ...
  // console.log("north values: x: " +optionNorthX+ ", y: " +htmlLinkScreenGridPositions[1]);
  //compareTilesForOptions(movementOptionNorth);
  let movementOptionEast = [htmlLinkScreenGridPositions[0], optionEastY];
  // console.log("east values: x: "+htmlLinkScreenGridPositions[0]+ ", y: " +optionEastY);
  
  
  let movementOptionSouth = [optionSouthX, htmlLinkScreenGridPositions[1]];
  // console.log("opt south x: " +optionSouthX+", html link screen grid positions 1: "+htmlLinkScreenGridPositions[1]);
  let movementOptionWest = [htmlLinkScreenGridPositions[0], optionWestY];
  // console.log("movement option west: " +movementOptionWest);
  //compareTilesForOptions(movementOptionNorth, movementOptionEast, movementOptionSouth, movementOptionWest);
...
function getOptionGridValues() { // convert movement options to tile definitions for defining walkability, aka pseudo collision detection
  ...
  // // console.log(optNorthArrayPosition);
  ...
  // // console.log("opt south array values: x: " +optSouthX+", y: "+optSouthY);
  optSouthArrayPosition = (optSouthX * 16) + optSouthY;
  // // console.log("south array position: " +optSouthArrayPosition);
  let optWestX = htmlLinkScreenGridPositions[0];
  let optWestY = htmlLinkScreenGridPositions[1] - 1;
  optWestArrayPosition = (optWestX * 16) + optWestY;
  // console.log(optWestArrayPosition);
...
//  console.log("array of tile option values to be converted to definitions: " + array);
  ...
  // // console.log("north option tile grid value to be defined: " +tileGridValues[array[0]]);
  let eastOptionTileDefinition = tileGridValues[array[1]];
  // // console.log("east option tile grid value to be defined: " +tileGridValues[array[1]]);
  let southOptionTileDefinition = tileGridValues[array[2]];
  // // console.log("west option tile grid value to be defined: " +tileGridValues[array[2]]);
...
//  console.log("option tile definitions. n: " +northOptionTileDefinition+ ", e: " +eastOptionTileDefinition+ ", s: " +southOptionTileDefinition+ ", w: " +westOptionTileDefinition);
  
  return optionTileValues;
}

function getTileDetails(string){
//  console.log("inside get tile details. string is: " + string);
...
//  console.log("array of tile values to be converted to tile definitions: " +array);
  // // console.log(gridTilesDictionary.gridTiles);
  let northTile = array[0];
  let northTileDefinition = gridTilesDictionary.gridTiles[northTile];
//  console.log("tile definition north: " +northTileDefinition);
  if(northTileDefinition == "tile-black") {
  	  getTileDetails(northTileDefinition);
  	  
  	//  console.log("door tile located! grid id is: " +htmlScreenGridTileID);
    //  console.log("screen grid position: " + htmlLinkScreenGridPositions);
  }
  let eastTile = array[1];
  let eastTileDefinition = gridTilesDictionary.gridTiles[eastTile];
//  console.log("tile definition east: " +eastTileDefinition);
  let southTile = array[2];
  let southTileDefinition = gridTilesDictionary.gridTiles[southTile];
//  console.log("tile definition south: " +southTileDefinition);
  let westTile = array[3];
  let westTileDefinition = gridTilesDictionary.gridTiles[westTile];
//  console.log("tile definition west: " +westTileDefinition);
...
function linkDirectionalCSSClasses(string){ // string passed in defines directional action link is taking; add that direction's class/remove all others.
  // // console.log("link directional css classes: " +string);
  // // console.log(htmlLink.classList);
...
function moveLinkNorth(){
  ...
  let directionalClass = "link-north";
  // // console.log("direction class" +directionClass);
...
function moveLinkAndTileNorth(){ // move link one current screen grid value north, while simultaneously changing the current screen to the one above (north) of it
  ...
  moveLinkTileJSON(northOptionJSONID, tileWildCard);
}
...
function linkMovementEast() {
	...
	//// // console.log("html link screen grid positions 1: " +htmlLinkScreenGridPositions[1]);
...
function moveLinkAndTileWest(){ // // console.log("move link and active current screen grid tile west!");
  tileWildCard = "west";
  var westOptionJSONID = tileOptions[3][0]+tileOptions[3][1];
  // // console.log("wjsonid: " +westOptionJSONID+ ", to0: " +tileOptions[3][0]+ ", to1: " +tileOptions[3][1]);
  
  moveLinkTileJSON(westOptionJSONID, tileWildCard);
}
function linkMovementWest() {
//  console.log("walkability west option: " +walkabilityOptions[3]+ ", and html link screen grid positions 1 is: " +htmlLinkScreenGridPositions[1]);
...
function locateLink(){ // locateLink function is center for link's functionality. all functions go through here.
  ...
  // // console.log("tile options: " +tileOptions);
  //  tileOptions[0]
  getScreenGridTileOptions(tileOptions);
  ...
  link = {
    ...
    movementOptions: movementOptions,
    hyruleAdjacentTiles: tileOptions,
    screenGridTileOptions: tileOptionValues,
    optionGridValues: optionTilePositionValues,
    optionTileValues: optionTileValues,
    optionTileDefitions: optionTileDefinitions,
    walkabilityScore: walkabilityOptions,
