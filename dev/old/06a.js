  ...
  function keyDown() {
    window.addEventListener("keydown", event => {
      ...
      if (event.key == "ArrowUp") {
        ...
        console.log("enter is start in nes, for selecting main/sub screen");
      } else if (event.key == "Shift") {
        console.log("shift key pressed");
        console.log("shift key is select in nes; pause/restart buttons");
      } else if (event.key == "z") {
        console.log("z key pressed!");
        console.log("z key is b in nes! for using and attacking with gadgets (treasures etc.)");
      } else if (event.key == "x") {
        console.log("x key pressed!");
        console.log("x key is a (attacking with sword) in nes");
      }
    });
  }
  function keyUp() { 
...
//var outputHTMLElement;
//var outputHTMLContent;
//var outputHTMLElement2;
//var outputHTMLContent2;
//var linkHTML;
var linkCSSClassList = [];
//var gridAspectRatio = 32;

function linkCSSClassUpdater(linkCSSClassList){
  console.log("classes are: " +linkCSSClassList);
...
function moveLinkNorth(){
  //console.log("event key in move link function: " +event.key);
  var currentPosition = linksPosition();
  //console.log("link's current position: " +currentPosition);
  var yoyoyoyo = getLinkCSSGridPositions(currentPosition);
  //console.log("01: " +yoyoyoyo[0]+ ", 1: " +yoyoyoyo[1]);
  var newCSSTopPosition = yoyoyoyo[0] - gridAspectRatio;
  //console.log("new css top position: " +newCSSTopPosition);
  var newCSSTopPositionValue = newCSSTopPosition + "px";
  linkHTML.style.setProperty("top", newCSSTopPositionValue);
  console.log("link's new moved position: " +linksPosition());
  
  //linkHTML.setProperty("top", newTop);
  // eventually: if((event.key == "ArrowUp") && (option north is true))
...
function keyArrowUpOnKeyDown() {
  ...
  console.log(linkHTML.classList.value);
}
