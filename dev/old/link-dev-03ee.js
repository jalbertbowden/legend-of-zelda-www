...
var tileHTMLParent = document.getElementById("hyrule-grid");
// dev only
var outputHTML;
...
function getOptionEast(){ //optionEastY = (parseInt(hyruleGridColumn) + 1).toString();
  optionEastY = htmlLinkScreenGridPositions[1] + 1; //console.log("option east val: " +optionEastY);
  return optionEastY;
}
function getOptionSouth(){ // optionSouthX = (parseInt(hyruleGridRow) + 1).toString();
  optionSouthX = htmlLinkScreenGridPositions[0] + 1;
  return optionSouthX;
}
function getOptionWest(){ // optionWestY = (parseInt(hyruleGridColumn) - 1).toString();
...
function convertOptionsToTileValues(array) { //console.log("array of tile option values to be converted to definitions: " + array);
  ...
  let eastOptionTileDefinition = tileGridValues[array[1]]; // console.log("east option tile grid value to be defined: " +tileGridValues[array[2]]);
  let southOptionTileDefinition = tileGridValues[array[2]]; //console.log("west option tile grid value to be defined: " +tileGridValues[array[3]]);
...
function createTileGridHTMLSCRIPT(string){ // pass in tile id of tile being generated
  console.log("string inside generate html script element for creating tile grid options: " +string);
  let scriptPath = "grid-tiles/" +string+ ".js";
  console.log("path of script to be loaded: " +scriptPath);
  let htmlScript = document.createElement("script");
  htmlScript.src = scriptPath;
  //console.log("this script: " +htmlScript);
  document.body.appendChild(htmlScript);
  console.log("new script has been appended!");
  htmlScript.onload = function() {
  	console.log("tile html parent: " +tileHTMLParent);
    let tileHTMLUL = document.getElementById("off-screen-grid-north");
    //let parentDiv = document.getElementById("ul-screen-grid").parentNode;
    //let listBullshitContent = document.getElementById("ul-screen-grid");
    
    // parentDiv.insertBefore(tileHTMLUL, listBullshitContent);
    
    //tileHTMLUL.id = "off-screen-grid-north";
    //tileHTMLUL.setAttribute("data-screen-grid-tile", string);
    //tileHTMLUL.classList.add("off-screen-grid");
    //tileHTMLUL.innerHTML = gridTileHTML0708;
    //document.getElementById("off-screen-grid-north").innerHTML = gridTileHTML0708;
    //tileHTMLParent.appendChild(tileHTMLUL);
    //parentDiv.insertBefore(tileHTMLUL, listBullshitContent);
    console.log("new markup for tile has been injected yo!");
  }
}

// north movement functions
function createOptionTile(string){ // pass in string indicating direction
  console.log("inside create option tile function: " +string);
  console.log("current tile id: " +htmlScreenGridTileID);
  console.log("movement options: " +movementOptions+ ", movement option north: " +movementOptions[0]);
  console.log("tile options: " +tileOptions+ ", tile options north: " +tileOptions[0]);
  
  //let createTileHTMLUL = document.createElement("ul");
  //createTileHTMLUL.id = "off-screen-grid-north";
  //createTileHTMLUL.classList.add("off-screen-grid");
  //let tileHTMLParent = document.getElementById("hyrule-grid");
  
  let createTileID = tileOptions[0][0] + tileOptions[0][1];
  //console.log("tile id to be created: " + createTileID);
  createTileGridHTMLSCRIPT(createTileID);
  // createTileHTMLUL.dataset.screenGridTile(createTileID);
  //createTileHTMLUL.setAttribute("data-screen-grid-tile", createTileID);
  //let xxxx = "hyruleGridTile" + createTileID;
  //console.log("xxxx: " +xxxx);
  //console.log("xxxx tester: " +hyruleGridTiles[xxxx]);
  //let tileToMake = hyruleGridTiles.xxxx;
  //console.log("tile to make: " +tileToMake+ ", hyrule grid tiles: " +hyruleGridTiles);
  
  //tileHTMLParent.appendChild(createTileHTMLUL);
  //console.log("new list injected!");
...
function linkMovementNorth(){ // move north options cycle
  if(walkabilityOptions[0] == true) { // if north option is walkable
    ...
    //console.log("current tile id: " +htmlScreenGridTileID);
    let tileOptionDirection = "north";
  	createOptionTile(tileOptionDirection);// create north grid on the fly
  	console.log("north tile created!");
  	let yourMOMdotCOM = document.getElementById("off-screen-grid-north");
  	console.log("your mom dot com: " +yourMOMdotCOM);
...
function moveLinkAndTileEast(){ // console.log("move link and active current screen grid tile east!");
...
function linkMovementEast() {
  if ((walkabilityOptions[1] == true) && (htmlLinkScreenGridPositions[1] < 15)) { //console.log("walkability option east is true!");
    moveLinkEast();
...
function moveLinkAndTileSouth(){ // console.log("move link and active current screen grid tile south!");
...
getHTMLDataAttributeValue(htmlScreenGrid); // console.log(htmlScreenGridTileID);
getHyruleGridPositions(htmlScreenGridTileID); // console.log("html screen grid tile id: " +htmlScreenGridTileID);
