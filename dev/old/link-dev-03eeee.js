...
var tileScripts = []; // array of tileScripts like "0808.js" OR IS IT?????
var tileScriptsContentAppendJS = ".js";
var tileScriptsContentPrependDirectory = "grid-tiles/";

function createTileHTMLScripts(){
  console.log("tile options from within create script function biznatch!" + tileOptions);
  console.log("ton: " +tileOptions[0]);
  console.log("ton1: " +tileOptions[0][1]);
  let scriptNorth;
  let scriptEast;
  let scriptSouth;
  let scriptWest;
  
  if(tileOptions[0][0] > 0) {
    scriptNorth = tileScriptsContentPrependDirectory+tileOptions[0][0]+tileOptions[0][1]+tileScriptsContentAppendJS;
  }
  if(tileOptions[1][1] < 15) {
    scriptEast = tileScriptsContentPrependDirectory+tileOptions[1][0]+tileOptions[1][1]+tileScriptsContentAppendJS;
  }
  if(tileOptions[2][0] < 8)  {
    scriptSouth = tileScriptsContentPrependDirectory+tileOptions[2][0]+tileOptions[2][1]+tileScriptsContentAppendJS;
  }
  if(tileOptions[3][1] > 0) {
  	scriptWest = tileScriptsContentPrependDirectory+tileOptions[3][0]+tileOptions[3][1]+tileScriptsContentAppendJS;
  }
  console.log("script vars so far! north: " +scriptNorth+ ", east: " +scriptEast+ ", south: " +scriptSouth+ ", west: " +scriptWest);
  // temporary script url: https://bowdenweb.com/nes/loz/dev/grid-tiles/0707.js
  // variable to seek: gridTileHTML0707
  let newHTMLScript;
  let newHTMLScripts = [scriptNorth, scriptEast, scriptSouth, scriptWest];
  
  
  for(var i = 0; i < tileOptions.length; i++){
  	if(newHTMLScripts[i] != null) {
    newHTMLScript = document.createElement("script");
    newHTMLScript.classList.add("screen-grid");
    document.body.appendChild(newHTMLScript);
    newHTMLScript.src = newHTMLScripts[i];
    console.log("script injected!");
    newHTMLScript.onload = function () {
      console.log("new script injection has loaded!");
      console.log("vars should be accessible punk!");
    }
    }
  }
  ...
  // check for existence of script elements of same
  let currentScripts = document.querySelectorAll("script.screen-grid"); // all screen grid scripts will be of class "screen-grid"
  //COME BACK TO THE ABOVE...YOU'RE GOING TO HAVE TO ACCOUNT FOR/MAKE SURE YOU'RE NOT INJECTING SAME SCRIPTS OVER AND OVER, ETC....
...
function getAdjacentHyruleTiles() { // get n, e, s, w hyrule tile grids adjacent to current active tile grid gameplay is on
  ...
  getOptionSouth();
  getOptionWest();
  //console.log("grid column: " +hyruleGridColumn+ ", grid row: " +hyruleGridRow);
  
  //let zeroOptionNorthX = 0 + optionNorthX.toString();
  //let zeroOptionEastY = 0 + optionEastY.toString();
  //let zeroOptionSouthX = 0 + optionSouthX.toString();
  //let zeroOptionWestY = 0 + optionWestY.toString();
  let zeroOptionNorthX = parseInt(hyruleGridRow) - 1;
  zeroOptionNorthX = 0 + zeroOptionNorthX.toString();
  //console.log("zero option north x: " +zeroOptionNorthX);
  let zeroOptionEastY = parseInt(hyruleGridColumn) + 1;
  zeroOptionEastY = 0 + zeroOptionEastY.toString();
  //console.log("zero option east y : " +zeroOptionEastY);
  let zeroOptionSouthX = parseInt(hyruleGridRow) + 1;
  zeroOptionSouthX = 0 + zeroOptionSouthX.toString();
  //console.log("zero option south x: " +zeroOptionSouthX);
  let zeroOptionWestY = parseInt(hyruleGridColumn) - 1;
  //console.log("zero option west y: " +zeroOptionWestY);
  zeroOptionWestY = 0 + zeroOptionWestY.toString();
  //console.log("zero option west y deuce: " +zeroOptionWestY);
  
  //console.log("zero option west y: " +zeroOptionWestY);
  
  
  tileOptionNorth = [zeroOptionNorthX, hyruleGridColumn];
  //console.log("tile option north: " +tileOptionNorth);
  ...
  //console.log("tile option west: " +tileOptionWest);
  tileOptions = [tileOptionNorth, tileOptionEast, tileOptionSouth, tileOptionWest];
  
  //console.log("tile options INITIAL: " +tileOptions);
  createTileHTMLScripts(); // create script elements consisting of screen grids' markup, to be injected if/when link moves position into a new tile
  return tileOptions;
}
...
function getOptionGridValues() { // convert movement options to tile definitions for defining walkability, aka pseudo collision detection
  ...
  optionTilePositionValues = [optNorthArrayPosition, optEastArrayPosition, optSouthArrayPosition, optWestArrayPosition];
  //console.log("opt tile position values: " +optionTilePositionValues);
  return optionTilePositionValues;
}
...
function moveLinkWest(){
  ...
  let newHTMLLinkCSSLeftPosition = htmlLinkCSSPositions[1] - 32; // console.log("new left position value to move to: " +newHTMLLinkCSSLeftPosition);
  let newHTMLLinkCSSLeftPositionValue = newHTMLLinkCSSLeftPosition + "px";
  htmlLink.style.left = newHTMLLinkCSSLeftPositionValue; // console.log("current direction class: " +directionClassCurrent);
...
**
**
**  screen grid tile generation - build screen grid's html on the fly - as link is actively trying to enter the new-soon-to-be-currently-active screen grid
**
**
**
**/
function screenGridHTML(string){ // accepts string value marking direction of new tile generation
  console.log("string var passed: " +string);
  console.log("current screen data attribute value screen grid id: " +htmlScreenGridTileID+ ", hyrule grid column: " +hyruleGridColumn+ ", hyrule grid row: " +hyruleGridRow);
...
function moveLinkAndTileNorth(){ // move link one current screen grid value north, while simultaneously changing the current screen to the one above (north) of it
  ...
  let genericVar = "north";
  screenGridHTML("north"); // generate north option tile's markup!
  ...
  screenGridCurrent.classList.add("off-screen-grid");
  screenGridNorth.id = "ul-screen-grid";
  screenGridCurrent.id = "off-screen-grid-south";
  // console.log("ids swapped!");
  screenGridNorth.appendChild(htmlLink);
...
function moveLinkAndTileEast(){
  console.log("move link and active current screen grid tile east!");
  let screenGridEast = document.getElementById("off-screen-grid-east");
  screenGridEast.classList.remove("off-screen-grid");
  let screenGridCurrent = document.getElementById("ul-screen-grid");
  screenGridCurrent.classList.add("off-screen-grid");
  screenGridEast.id = "ul-screen-grid";
  screenGridCurrent.id = "off-screen-grid-west";
  screenGridEast.appendChild(htmlLink);
...
function moveLinkAndTileSouth(){
  // console.log("move link and active current screen grid tile south!");
  let screenGridSouth = document.getElementById("off-screen-grid-south");
  let screenGridCurrent = document.getElementById("ul-screen-grid");
  screenGridSouth.id = "ul-screen-grid";
  screenGridSouth.classList.remove("off-screen-grid");
  screenGridCurrent.classList.add("off-screen-grid");
  screenGridCurrent.id = "off-screen-grid-north";
  // console.log("ids swapped!");
  screenGridSouth.appendChild(htmlLink);
...
function moveLinkAndTileWest(){
  console.log("move link and active current screen grid tile west!");
  let screenGridWest = document.getElementById("off-screen-grid-west");
  let screenGridCurrent = document.getElementById("ul-screen-grid");
  screenGridWest.id = "ul-screen-grid";
  screenGridWest.classList.remove("off-screen-grid");
  screenGridCurrent.classList.add("off-screen-grid");
  screenGridCurrent.id = "off-screen-grid-east";
  screenGridWest.appendChild(htmlLink);
