...
function getOptionGridValues() { // convert movement options to tile definitions for defining walkability, aka pseudo collision detection
  ...
  // console.log("opt north x: " +optNorthX+ ", opt north y: " +optNorthY);
...
function moveLinkWest(){
  ...
  let newHTMLLinkCSSLeftPositionValue = newHTMLLinkCSSLeftPosition + "px";
  htmlLink.style.left = newHTMLLinkCSSLeftPositionValue;
  // console.log("new left position value to move to: " +newHTMLLinkCSSLeftPosition+ ", current direction class: " +directionClassCurrent);  
  //htmlLink.classList.toggle(directionClassCurrent);  
...
function moveLinkTileJSON(string){ // accepts tile option string of calling direction; ajax call is made for screen grid JSON of tile moving into; returns json response of html chunk in ul, representing the selected tile grid
  var requestJSONURL = "grid-tiles/"+string+".js";
  // console.log("request JSON URL var: " +requestJSONURL);
  ...
  request.onload = function() {
    ...
    // console.log("screen grid tile: " +screenGridTile);
    populateGridTileScreen(screenGridTile);
  }
}
function moveLinkAndTileNorth(){ // move link one current screen grid value north, while simultaneously changing the current screen to the one above (north) of it
  ...
  var northOptionJSONID = tileOptions[0][0]+tileOptions[0][1];
  console.log("north option json id: "+northOptionJSONID);
  moveLinkTileJSON(northOptionJSONID);
  
  
  //var currentGrid = document.getElementById("ul-screen-grid");
  //currentGrid.appendChild(htmlLink);
  //htmlLink.style.setProperty("top", "320px");
  console.log("north option json id: " +northOptionJSONID);
  
  console.log("pre link.tileGrid value: " +link.tileGrid);
  
  // htmlScreenGridTileID = northOptionJSONID;
  link.tileGrid = northOptionJSONID;
  
  console.log("new html screen grid tile id: " +link.tileGrid);
  
  let newCSSPositionValues = getCSSPositionValues(htmlLink);
  console.log("new css posiiton values: " +newCSSPositionValues);
  //locateLink();
  updateLink(newCSSPositionValues);
  // return htmlScreenGridTileID;
}
function populateGridTileScreen(jsonObj) {
  // 
  var htmlLink = document.getElementById("link");
  ...
  var outputDiv = document.getElementById("screen-grid");
  outputDiv.innerHTML = jsonObj["gridTile"];
  getHTMLDataAttributeValue(htmlScreenGrid);
  console.log("New html screen grid tile id via json son!"+htmlScreenGridTileID);
  
  
  var newGridTileList = document.getElementById("ul-screen-grid");
  newGridTileList.appendChild(htmlLink);
  htmlLink.style.setProperty("top", "320px");
  htmlScreenGridTileID = newGridTileList.dataset.screenGridTile;
  console.log("htlm screen grid tile id: " +htmlScreenGridTileID);
  let newCSSPositionValues = getCSSPositionValues(htmlLink);
  
  console.log("json injection aka population function complete!");
  updateLink();
}
...
function linkMovementEast() {
	...
	// console.log(" 1: " +htmlLinkScreenGridPositions[1]+ ", hyruleGridColumn value: " +hyruleGridColumn);
...
function linkMovementWest() {
  ...
  if((walkabilityOptions[3] ==  true) && (htmlLinkScreenGridPositions[1] > 0)){
    moveLinkWest();
  	...
  	// console.log("move tile west functionality triggered!");
    moveLinkAndTileWest();
